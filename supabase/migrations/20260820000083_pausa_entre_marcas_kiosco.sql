-- En el kiosco, la misma cara dos veces seguidas no es otra marca.
--
-- En planta la gente se para, ficha, y a veces sigue en el óvalo (o
-- se vuelve a asomar a los dos segundos). El RPC alterna ingreso/egreso
-- según la última marca: esa segunda mirada se registraba como salida.
-- No es que se hayan ido: no se movieron de la tablet.
--
-- La salida no depende de un horario ("después de las 17"). Si se van
-- antes, vuelven a mirar la cámara y es egreso, a la hora que sea. Lo
-- que no puede pasar es un doble toque inmediato. Por eso el 1:N, si
-- esa persona ya fichó hace menos de 2 minutos, **devuelve la marca
-- que ya está** y no inserta otra. El 1:1 (celular propio) no tiene
-- esta pausa: ahí hay un botón, no una cola.
--
-- No cambia la firma.
...
begin
  ...
  if p_empleado_id is null then
    ...
    -- Misma persona, hace un momento: no es otra marca. Devolver la
    -- que ya está evita el falso egreso de quien sigue en el óvalo.
    select f.*
      into v_fila
      from fichajes f
     where f.empleado_id = v_mejor.id
       and f.anulado_en is null
       and f.ts > clock_timestamp() - interval '2 minutes'
...
comment on function public.fichar_con_rostro is
  ...
  'tenant es el de la terminal (F-06). 1:1 exige ser el titular. En '
  '1:N, una marca de la misma persona de hace menos de 2 minutos se '
  'devuelve tal cual: no es un egreso, es la cara que no se fue.';
