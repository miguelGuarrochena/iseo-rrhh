-- F-06: el kiosco del superadmin puede fichar.
--
-- El problema
-- -----------
-- `fichar_con_rostro` cortaba con "Sin empresa activa" si
-- `auth_empresa()` era NULL. Eso es siempre cierto para el dueño de
-- ISEO: `usuarios.empresa_id` es NULL a propósito, no está atado a
-- ningún cliente.
--
-- La UI, en cambio, cree que sí puede. `ModoKiosco` guarda la empresa
-- en localStorage y llama a `entrarAEmpresa()`, que sólo muta el store
-- del navegador. El servidor no se entera. Resultado: la cámara
-- captura, el RPC rechaza, la pantalla reinicia el intento. En planta
-- se veía como un bucle eterno de "parpadeá".
--
-- Por qué la empresa sale de la terminal, no de un parámetro
-- ----------------------------------------------------------
-- La alternativa fácil sería `p_empresa_id`, como en
-- `autorizar_terminal` (migración 79). Acá no: el 1:N ya manda el par
-- (terminal, secreto), y esa credencial **ya está atada a una empresa**.
-- Pedir la empresa otra vez sería dejar que el cliente la afirme. El
-- superadmin podría fichar en cualquier tenant con un uuid; un
-- admin_rrhh no, porque seguiríamos cruzando con `auth_empresa()`, pero
-- el superadmin sí.
--
-- Así que:
--
--   * 1:1 (celular): sigue exigiendo `auth_empresa()`. El empleado
--     pertenece a una empresa; si no hay, no hay a quién fichar.
--   * 1:N (kiosco) con empresa en el JWT: igual que antes,
--     `terminal_habilitada` contra `auth_empresa()`.
--   * 1:N con superadmin: la empresa es la de la terminal, y sólo si
--     el secreto coincide. Un id de terminal sin el secreto no revela
--     de qué empresa es (mismo criterio que `terminal_habilitada`: no
--     oráculo).
--
-- No cambia la firma. No hay que dropear: un `create or replace` con
-- los mismos argumentos reemplaza. F-01 sigue intacto (hace falta
-- gestor + terminal vinculada). El cruce de tenant de un admin_rrhh
-- con la tablet de otro cliente sigue cortando en
-- `terminal_habilitada`.
